...
/// <summary>
/// Bounded, opt-in character selection for the disposable local Valheim lab.
/// This restores only the smallest unattended seam: select an existing profile and
/// invoke Valheim's normal OnCharacterStart path. It does not create worlds, teleport,
/// run arbitrary scripts, or drive P7. COMFY_AUTOJOIN/config is deliberately off by
/// default and is intended for profile-gated lab clients only.
/// </summary>
public static class LabAutoJoinPatches
{
    ...
    private static void FejdStartupReadyPostfix(FejdStartup instance)
    {
        if (!IsEnabled() || _armed || _completed || instance == null) return;
        _armed = true;
        instance.StartCoroutine(Drive(instance));
    }
    ...
    {
        string requestedName = FirstNonEmpty(Environment.GetEnvironmentVariable("COMFY_AUTOJOIN_CHARACTER"), PluginConfig.AutoJoinCharacterName.Value);
